...
export const checkStartTime: ValidatorFn = (
...
export const checkEndTime: ValidatorFn = (
  control: AbstractControl
): ValidationErrors | null => {
  const startTime = new Date(control.get("startTime").value).getTime();
  ...
  console.log(endTime);
  if (
    (endTime - startTime > 30 * 24 * 60 * 60 * 1000 && startTime && endTime) ||
    endTime - startTime < 0
  ) {
...
export class AuctionProductAddComponent implements OnInit {
  ...
  addNewProduct() {
    ...
    this._productService.addProduct(this.productDto).subscribe(
      (data) => {
        ...
        this._toast.success(
          "Chúc mừng bạn đã đấu giá thành công, đợi bộ phận kiểm duyệt xác nhận và thông báo bạn sau nhé !",
          "",
          {
            timeOut: 3000,
            progressBar: true,
      ...
      (error) => {
        this._toast.error(
          "Yêu cầu của bạn không được duyệt, vui lòng nhập chính xác điều kiện đấu giá"
        );
      }
    );
  }
